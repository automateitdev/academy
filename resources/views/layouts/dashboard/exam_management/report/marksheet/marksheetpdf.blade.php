...
<head>
    ...
    <div style="height:90%; padding:30px; margin:40px; border:6px groove burlywood; overflow:auto;" class="pagebreak">
        <div style="text-align:center">
            <h1 style="text-transform: capitalize; margin:0 auto">
                <b>{{ $variable['common_detail']['institute_name'] }}</b>
            </h1>
            <p style="text-transform: capitalize; margin:0 auto; font-style: italic;">
                {{ $variable['common_detail']['institute_add'] }}</p>
            <img src="data:image/png;base64,{{ base64_encode(file_get_contents(storage_path('images/' . $variable['common_detail']['institute_logo']))) }}"
        ...
        <div style="overflow:auto">
            ...
            <div style="float:right; clear:both">
                <table style="text-align:left" border="1" cellspacing="5">
                    <tbody>
                        <tr>
                            <th style="padding:10px 20px">Range</th>
                            <th style="padding:10px 20px">Grades</th>
                            <th style="padding:10px 20px">Grade Points</th>
                        </tr>
                        <tr>
                            <td>80-100</td>
                            <td>A+</td>
                    ...
                    @foreach ($variable as $key => $item)
                        @if ($key != 'subject_details' && $key != 'common_detail')
                            @foreach ($item['subjects'] as $subname => $subjects)
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center">
                                        {{ $variable['subject_details'][$subname]['fullmarks'] }}</td>
                                    @php
                                    ...
                                    @foreach ($variable['subject_details'][$subjectTospan] as $examcode)
                                        @foreach ($subjects as $sub_info_key => $sub_info_detail)
                                            @if ($examcode == $sub_info_key)
                                                @php $subjectExamCode++ @endphp
                                                <td style="text-align:center">{{ $sub_info_detail }}</td>
                                            @endif
                                        @endforeach
                                    @endforeach

                                    @php $td_dashed = $colspan - $subjectExamCode @endphp


                                    @for ($count_td = 0; $count_td < $td_dashed; $count_td++)
                            ...
                            <tr style="background-color:seagreen;">
                                ...
                                <td style="text-align:center; color:white" colspan={{ $colspan }}>Obtained Marks
                                    and GPA</td>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="padding:10px">
                            {{ isset($variable[$studentID]['gpa_except_opt']) ? $variable[$studentID]['gpa_except_opt'] : '-' }}
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="2" cellpadding="0">
                            <textarea name="" id="" rows="5"
                                style="width: 100%; background:transparent; padding:20px 0; margin:0"></textarea>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div style="position:absolute; width:90%; left:5%; bottom:50; text-align:left; overflow:auto;">
            <p
                style="width:200px; display:inline-block; text-align:center; border-top: 1px dashed gray; margin:5px; padding: 60px; padding-top: 0; padding-bottom: 0; margin-top: 100px;">
                Class Teacher</p>
            <p
                style="width:200px; display:inline-block; text-align:center; float:right; border-top:1px dashed gray; margin:5px; padding: 60px; padding-top: 0;
